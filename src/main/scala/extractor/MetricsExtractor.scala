...
case class MetricsExtractor(target: ExecutorService) {
  val submittedTasksMethod: MethodHandle = getMethodHandle(target, "submittedTasks")
  val completedTasksCounterMethod: MethodHandle = getMethodHandle(target, "completedTasksCounter")
